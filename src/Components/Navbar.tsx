...
export default function Navbar() {
  ...
  return <><header className="sticky top-0 z-40 border-b bg-white/95 backdrop-blur">
    <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-4">
      ...
      <form onSubmit={search} className="hidden min-w-0 max-w-lg flex-1 md:flex"><input aria-label="Search courses" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search courses" className="min-w-0 w-full rounded-l-md border border-r-0 px-3 py-2 text-sm"/><button className="rounded-r-md bg-blue-700 px-3 text-white" aria-label="Search"><Search size={17}/></button></form>
      <nav className="ml-auto hidden items-center gap-3 md:flex"><Link href="/catalog" className="text-xs font-semibold">Courses</Link>{user?<><Link href="/profile" className="rounded-md border px-3 py-2 text-xs font-semibold">{user.name.split(" ")[0]}</Link><button onClick={logout} className="text-xs font-semibold text-slate-600">Log out</button></>:<button onClick={()=>setAuth(true)} className="rounded-md bg-blue-700 px-3 py-2 text-xs font-bold text-white">Join free</button>}</nav>
      <button onClick={()=>setMobile(!mobile)} className="ml-auto md:hidden" aria-label="Toggle navigation">{mobile?<X size={21}/>:<Menu size={21}/>}</button>
    </div>
